...
<div class="card shadow layout-card">
  <div class="card-body">
    <div class="row">
    @foreach($form_update_payment as $data)
      ...
      <div class="col-lg-6 col-md-6 col-sm-12 col-12 layout-app">
        <center>
          <span>Slip</span>
          <div>
            <img src="{{ asset('upload/payment/'.$data->payment_file) }}" width="100px" />
          </div>
        </center>
        <form method="POST" action="{{ url('/confirm-update-payment/'.$data->id) }}" enctype="multipart/form-data">
        {{ csrf_field() }}
          ...
          <center>
            <button type="submit" class="btn btn-app">อัพเดทการชำระ</button>
          </center>
        </form>
        <hr />
        <div class="card">
